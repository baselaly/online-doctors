...
<v-container>
  ...
  <v-card>
    <v-card-title>
      <v-chip color="white" large text-color="black">checked Appointments</v-chip>
...
export default {
  ...
  methods: {
    ...
    getCheckedAppointments() {
      var token = localStorage.getItem("token");
      axios
        .get("/api/admin/appointment/checked", {
    ...
    appointmentsPagination() {
      ...
      var token = localStorage.getItem("token");
      axios
        .get(`/api/admin/appointment/checked?page=${this.page}`, {
          headers: { Authorization: "Bearer " + token }
        })
        .then(response => {
          var response = response.data.appointments;
