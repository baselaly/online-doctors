...
<v-container>
  ...
  <v-card>
    <v-card-title>
      <v-chip color="white" large text-color="black">unchecked Appointments</v-chip>
...
export default {
  ...
  methods: {
    ...
    getUncheckedAppointments() {
      ...
      axios
        .get("/api/admin/appointment/unchecked", {
    ...
    appointmentsPagination() {
      ...
      var token = localStorage.getItem("token");
      axios
        .get(`/api/admin/appointment/unchecked?page=${this.page}`, {
          headers: { Authorization: "Bearer " + token }
        })
        .then(response => {
          var response = response.data.appointments;
