                  ...
                  <div>
                    ...
                    <div class="body-2 py-1 text-truncate">
                      <router-link
                        style="color:black;"
                        :to="{ name: 'doctor', params: {id: appointment.doctor.id, name: appointment.doctor.name } }"
                      >Dr. {{appointment.doctor.name}}</router-link>
                    </div>
                    <div class="caption py-1">Fees : {{appointment.doctor.fees}} EGP</div>
                    <div class="caption py-1 text-truncate">
                      <v-icon size="15" color="black">phone</v-icon>
...
<script>
import cancelDialog from "./booking_dialogs/cancelDialog";
...
export default {
  ...
  methods: {
    ...
    appointmentsPagination() {
      ...
      var token = localStorage.getItem("token");
      axios
        .get(`/api/user/upcoming?page=${this.page}`, {
          headers: {
            Authorization: "Bearer " + token
          }
        })
