<template>
  <v-flex>
    ...
    <v-layout fill-height px-5 py-5 text-xs-center v-if="no_appointments">
      <v-flex xs12>
        <span class="headline">No Appointments Today</span>
      </v-flex>
    </v-layout>
  ...
  methods: {
    getToday() {
      ...
      axios
        ...
        .then(response => {
          var response = response.data.today_appointments;
          this.appointments = response.data;
          this.handlePagination(response);
          if (this.appointments.length == 0) {
            this.activateSnackbar(true, "red", "no appointments today");
    ...
    appointmentsPagination() {
      ...
      var token = localStorage.getItem("token");
      axios
        .get(`/api/doctor/today?page=${this.page}`, {
          headers: {
            Authorization: "Bearer " + token
          }
        })
